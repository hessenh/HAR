...
def extract_merged_labels(subject, output_size):
  filepath = '../Prosjektoppgave/Notebook/data/'+subject+'/DATA_WINDOW/1.5/ORIGINAL/Usability_LAB_All_NEW_TRANSITION_L.csv'
...
def extract_labels(subjects, output_size):
  print('Extracting label from', subjects)
  train_label = extract_merged_labels(subjects[0], output_size)

  for i in range(1,len(subjects)):
    subject_label = extract_merged_labels(subjects[i], output_size)
...
def read_data_sets(subjects_set, output_size):
  ...
  train_labels = extract_labels(training_subjects, output_size)

  # Testing data and labels
  test_data = extract_data(test_subjects)
  test_labels = extract_labels(test_subjects, output_size)
